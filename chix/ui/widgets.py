...
from chix.utils.code_completion import CodeCompleter, CompletionPopup
from chix.utils.debugger import BreakpointManager, DebuggingSession
import webbrowser
import re
import os
...
class EnhancedTextEditor(ctk.CTkTextbox):
    ...
    def __init__(self, master, state, **kwargs):
        ...
        self.completion_popup = CompletionPopup(self._textbox, self.code_completer)
        
        # If current project directory is set, scan project files for better completion
        if "current_directory" in state and state["current_directory"]:
            self.code_completer.scan_project_files(state["current_directory"])
            
        # Initialize breakpoint management
        self.breakpoint_manager = BreakpointManager(self)
        self.debugging_session = DebuggingSession(self, self.breakpoint_manager)
        ...
        # Enable Xcode-style UI appearance
        self._apply_xcode_style()
    ...
    def _setup_events(self):
        ...
        # Code completion
        self.bind('<Control-space>', self._show_code_completion)
        self.bind('period', self._auto_show_completion_on_dot)
        self.bind('<Escape>', self._hide_code_completion)
        
        # Track changes
        self.bind('<KeyRelease>', self._on_text_changed)
    
    def _apply_xcode_style(self):
        """Apply Xcode-like styling to the editor"""
        # Configure text widget for Xcode-like appearance
        self._textbox.configure(
            insertwidth=2,                      # Wider cursor like Xcode
            insertbackground=get_color("accent_primary"),  # Cursor color
            selectbackground=get_color("selection"),       # Selection color
            padx=10,                           # Add padding on the sides
            pady=5                             # Add padding on top/bottom
        )
        
        # Apply a subtle border
        self.configure(
            border_width=1,
            border_color=get_color("bg_secondary")
        )
    
    def _show_code_completion(self, event=None):
        """Show code completion popup"""
        self.completion_popup.show()
        return "break"
    
    def _hide_code_completion(self, event=None):
        """Hide code completion popup"""
        self.completion_popup.hide()
        return None
    
    def _auto_show_completion_on_dot(self, event=None):
        """Automatically show completion after typing a dot"""
        # Insert the dot first
        self.insert(tk.INSERT, ".")
        # Then show completion with a slight delay
        self.after(100, self._show_code_completion)
        return "break"
    ...
    def _setup_tags(self):
        ...
        text_widget.tag_configure("error", foreground=get_color("error"), underline=True)
